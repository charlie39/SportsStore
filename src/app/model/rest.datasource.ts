...
export class RestDataSource {
	...
	//	this.baseUrl = `${PROTOCOL}://${location.hostname}:${PORT}/`; -> for seperate server
		this.baseUrl ="/api/";   //for same server
	}
	...
	authenticate(user: string, pass: string): Observable<boolean> {
		return this.http.post<any>(this.baseUrl + "login", {
			...
			.pipe(map(response => {
				...
				//console.log(response.success); -> for development
				//console.log(response.token);    -> testing
